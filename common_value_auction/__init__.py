...
def creating_session(subsession: Subsession):
    for g in subsession.get_groups():
        ...
        item_value = random.uniform(Constants.min_allowable_bid, Constants.max_allowable_bid)
...
class Introduction(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        player.player.item_value_estimate = player.group.generate_value_estimate()
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        return dict(is_greedy=player.group.item_value - player.player.bid_amount < 0)
